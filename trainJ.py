...
@author: fangshuyang (yfs2016@hit.edu.cn) changed by jmtargino
...
for epoch in range(Epochs): 
    ...
    for i in range(1,TrainSize,pulo): #vai dando um pulo de 100 em 100
        train_set,label_set,data_dsp_dim,label_dsp_dim  = DataLoad_Train(train_size=pulo,train_data_dir=train_data_dir, \
                                                                     data_dim=DataDim,in_channels=Inchannels, \
                                                                     model_dim=ModelDim,data_dsp_blk=data_dsp_blk, \
                                                                     label_dsp_blk=label_dsp_blk,start=i, \
    ...
    # Change data type (numpy --> tensor)
        ...
        train_loader = data_utils.DataLoader(train,batch_size=BatchSize,shuffle=True) #conjunto com shuffle
        ...
        print ('Training size:%d'                %  int(pulo))
